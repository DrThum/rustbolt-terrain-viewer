...
export function drawTerrain(blockOffsetX: number, blockOffsetZ: number, index: number, terrain: TerrainGeometry) {
  const chunkOffsetX = blockOffsetX + (constants.CHUNK_WIDTH * (index % 16))
  const chunkOffsetZ = blockOffsetZ + (constants.CHUNK_WIDTH * Math.floor(index / 16))

  const planeGeom = new THREE.PlaneGeometry(constants.CHUNK_WIDTH, constants.CHUNK_WIDTH, 16, 16)
  planeGeom.rotateX(-Math.PI * 0.5)

  for (let i = 0; i < planeGeom.attributes.position.count; i++) {
    (planeGeom.attributes.position as THREE.BufferAttribute).setY(i, terrain.heightMap[i] + terrain.baseHeight)
  }
