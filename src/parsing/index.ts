...
export async function loadBlock(mapName: string, row: number, col: number): Promise<TerrainGeometry[]> {
  ...
  const promise: Promise<TerrainGeometry[]> = new Promise(resolve => resolvePromise = resolve)
  ...
  reader.addEventListener('loadend', () => {
    ...
    if (buffer instanceof ArrayBuffer) {
      ...
      const debugName = textDecoder.decode(buffer.slice(8, -1)).split('\0').shift() ?? ''

      const terrains: TerrainGeometry[] = []
      let currentOffset = 4 + 4 + debugName.length + 1
      for (let currentChunkNumber = 0; currentChunkNumber < 256; currentChunkNumber++) {
        const dataView = new DataView(buffer.slice(currentOffset))
        const baseHeight = dataView.getFloat32(12, true)
        const heightMapView = new Float32Array(buffer.slice(currentOffset + 20, currentOffset + 20 + (145 * 4)))
...
function interpolateHeightMap(heightMap: number[]): number[] {
  ...
  return heightMap.map((h, idx) => {
    if (((idx - 8) % 17) == 0) { // End of outer verticex row (8, 25, 42, ...): nothing to interpolate
      return [h]
    } else if (idx % 17 < 8) { // Outer vertices row (0-7, 17-24, ...): interpolate the mean between the current point and the next one
      const mean = (h + heightMap[idx + 1]) / 2
      return [h, mean]
    } else { // Inner vertices (9-16, 26-33, ...)
      if (idx % 17 == 9) { // First inner vertex of the row, interpolate before and after
        return [
          (heightMap[idx - 9] + heightMap[idx + 8]) / 2,
          h,
          (heightMap[idx - 8] + heightMap[idx + 9]) / 2
        ]
      } else { // Other vertices, only interpolate after
        return [
          h,
          (heightMap[idx - 8] + heightMap[idx + 9]) / 2
        ]
